...
# 1. PUBLIC API
# ---------------------------------------------------------------------------
# This section contains the primary functions called by the main operation.
    ...
    for z_target, cutArea, status in stack:
        ...
        if start_w_idx < len(working_area.Wires):
            for w_idx in range(start_w_idx, len(working_area.Wires)):
                ...
                start_p = _find_start_point(wire, start_point, cut_climb)
...
# ---------------------------------------------------------------------------
# 2. GEOMETRIC STACK GENERATION HELPERS
# ---------------------------------------------------------------------------
# Internal functions used exclusively by zlevel_hybrid_stack.
...
# ---------------------------------------------------------------------------
# 3. G-CODE GENERATION HELPERS
# ---------------------------------------------------------------------------
# Internal functions used exclusively by zlevel_hybrid_to_gcode.


def _find_start_point(wire, start_point, cut_climb):
    """
    Calculates the starting vertex based on either a user-provided
    start point or the milling direction (Climb vs. Conventional).

    Args:
        wire (Part.Wire): The input wire to process.
        start_point (FreeCAD.Vector): A user-defined preference for the start location, or None.
        cut_climb (bool): True for Climb milling, False for Conventional.

    Returns:
        tuple: A tuple containing (
            FreeCAD.Vector: The calculated starting point
        )
    """
...
# ---------------------------------------------------------------------------
# 4. UTILITY & PREPARATION HELPERS
# ---------------------------------------------------------------------------
# Internal helpers for the data preparation and categorization functions.


# -- Helpers for categorize_floor_steps --
