...
__title__ = "CAM Surface 3D Operation"
...
try:
    ...
    msg = translate("CAM_Surface3D", "This operation requires OpenCamLib to be installed.")
...
class ObjectSurface3D(PathOp.ObjectOp):
    ...
    def propertyEnumerations(cls, dataType="data"):
        ...
        enums = {
            ...
            "BoundBox": [
                ...
                (translate("CAM_Surface3D", "Stock"), "Stock"),
            ],
            "PatternCenterAt": [
                (translate("CAM_Surface3D", "CenterOfMass"), "CenterOfMass"),
    ...
    def opPropertyDefaults(self, obj, job):
        ...
        if warn:
            msg = translate("CAM_Surface3D", "The GeometryTolerance for this Job is 0.0.")
            msg += translate("CAM_Surface3D", "Initializing LinearDeflection to 0.001 mm.")
    ...
    def apply_accuracy_preset(self, obj, level):
        ...
        Args:
            obj: The Surface3D operation object
    ...
    def _extractToolParams(self, obj):
        ...
        Path.Log.info(
            "Surface3D tool: type={}, diameter={}, edge_height={}, "
    ...
    def opExecute(self, obj):
        """Main execution method for Surface3D operation.
        ...
        JOB = PathUtils.findParentJob(obj)
        if JOB is None:
            Path.Log.error(translate("CAM_Surface3D", "No JOB"))
            return
        ...
        if hasattr(obj, "Base") and obj.Base and not selected_faces:
            ...
            if has_face_selections:
                ...
                Path.Log.warning(
                    "Surface3D: User selected faces but all were avoided by AvoidLastX_Faces. Using whole model."
                )
        ...
        if cutter is None:
            Path.Log.error(
                translate("CAM_Surface3D", "Error creating OCL cutter from tool parameters.")
            )
            return
        tool_diam = cutter.getDiameter()
        Path.Log.info(
            "Surface3D OCL cutter created: getDiameter()={}, StepOver={}%, "
        ...
        stl = None
        for base, subs in self.baseShapes(obj):
            # For Surface3D, we only care about the base shape, not subs
        ...
        if stl is None:
            Path.Log.error(translate("CAM_Surface3D", "Failed to create STL from model."))
            return
        ...
        elapsed = time.time() - startTime
        Path.Log.info("Surface3D operation completed in {:.2f}s".format(elapsed))


def Create(name, obj=None, parentJob=None):
    """Create(name) ... Creates and returns a Surface3D operation."""
    if obj is None:
