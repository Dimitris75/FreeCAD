...
__title__ = "CAM 3D Surface Exp Operation"
...
try:
    ...
    msg = translate("CAM_SurfaceExp", "This operation requires OpenCamLib to be installed.")
...
class ObjectSurface(PathOp.ObjectOp):
    ...
    def propertyEnumerations(cls, dataType="data"):
        ...
        enums = {
            "Strategy": [
                (translate("CAM_SurfaceExp", "Surface Scan"), "SurfaceScan"),
                (translate("CAM_SurfaceExp", "Waterline"), "Waterline"),
                (translate("CAM_SurfaceExp", "Z-Level Hybrid"), "ZLevelHybrid"),
            ],
            "BoundBox": [
                (translate("CAM_SurfaceExp", "BaseBoundBox"), "BaseBoundBox"),
                (translate("CAM_SurfaceExp", "Stock"), "Stock"),
            ],
            "PatternCenterAt": [
                (translate("CAM_SurfaceExp", "Center of Boundary"), "CenterOfBoundary"),
                (translate("CAM_SurfaceExp", "Custom"), "Custom"),
            ],
            "CutMode": [
                (translate("CAM_SurfaceExp", "Conventional"), "Conventional"),
                (translate("CAM_SurfaceExp", "Climb"), "Climb"),
            ],
            "CutPattern": [
                (translate("CAM_SurfaceExp", "Line"), "Line"),
                (translate("CAM_SurfaceExp", "ZigZag"), "ZigZag"),
                (translate("CAM_SurfaceExp", "Circular"), "Circular"),
                (translate("CAM_SurfaceExp", "CircularZigZag"), "CircularZigZag"),
                (translate("CAM_SurfaceExp", "Spiral"), "Spiral"),
                (translate("CAM_SurfaceExp", "Offset"), "Offset"),
            ],
            "CutPatternZLevel": [
                (translate("CAM_SurfaceExp", "None"), "None"),
                (translate("CAM_SurfaceExp", "Line"), "Line"),
                (translate("CAM_SurfaceExp", "ZigZag"), "ZigZag"),
                (translate("CAM_SurfaceExp", "Offset"), "Offset"),
                (translate("CAM_SurfaceExp", "Adaptive"), "Adaptive"),
                (translate("CAM_SurfaceExp", "Grid"), "Grid"),
            ],
            "LayerMode": [
                (translate("CAM_SurfaceExp", "Single-pass"), "Single-pass"),
                (translate("CAM_SurfaceExp", "Multi-pass"), "Multi-pass"),
            ],
            "SamplingAccuracy": [
                (translate("CAM_SurfaceExp", "Standard"), "4"),
                (translate("CAM_SurfaceExp", "High"), "8"),
                (translate("CAM_SurfaceExp", "Very High"), "16"),
                (translate("CAM_SurfaceExp", "Ultra"), "32"),
            ],
            "HandleMultipleFeatures": [
                (translate("CAM_SurfaceExp", "Collectively"), "Collectively"),
                (translate("CAM_SurfaceExp", "Individually"), "Individually"),
            ],
            "ProfileEdges": [
                (translate("CAM_SurfaceExp", "None"), "None"),
                (translate("CAM_SurfaceExp", "First"), "First"),
                (translate("CAM_SurfaceExp", "Last"), "Last"),
                (translate("CAM_SurfaceExp", "Only"), "Only"),
            ],
            "AdaptiveAccuracy": [
                (translate("CAM_SurfaceExp", "Very Low"), "0.15"),
                (translate("CAM_SurfaceExp", "Low"), "0.1"),
                (translate("CAM_SurfaceExp", "Standard"), "0.08"),
                (translate("CAM_SurfaceExp", "High"), "0.05"),
                (translate("CAM_SurfaceExp", "Very High"), "0.02"),
            ],
        }
    ...
    def opUpdateDepths(self, obj):
        if hasattr(obj, "Base") and obj.Base:
            # Start from a neutral sentinel and let every face (including the
            # first) go through the same try/except path below.
            zmin = float("inf")
    ...
    def _executeSurfaceScan(
        ...
        # Normalize to a local, independent list up front. This does two things:
        # 1. Makes the documented `cutting_faces=None` default actually safe to
        #    call with — previously, a real None here would hit `.append()`
        #    below and raise AttributeError; only the single existing call site
        #    (which always passes a pre-populated list) avoided that.
        # 2. Avoids mutating a list the caller still holds a reference to.
    ...
    def opExecute(self, obj):
        ...
        JOB = PathUtils.findParentJob(obj)
        if JOB is None:
            Path.Log.error(translate("CAM_SurfaceExp", "No JOB"))
            return
        ...
        if needs_ocl_cutter:
            ...
            if cutter is None:
                Path.Log.error(
                    translate("CAM_SurfaceExp", "Error creating OCL cutter from tool parameters.")
                )
                return
